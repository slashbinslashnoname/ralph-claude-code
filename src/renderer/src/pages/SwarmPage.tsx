      ...
      {stats && stats.total > 0 && (
        <div className="stats-bar">
          ...
          <div className="stats-numbers">
            ...
            {telegramStatus && (
              <span
                className={`stat-chip telegram-status ${telegramStatus.connected ? 'connected' : 'disconnected'}`}
                title={telegramStatus.connected
                  ? `Telegram: @${telegramStatus.botUsername ?? 'unknown'}\nSent: ${telegramStatus.messagesSent} | Received: ${telegramStatus.messagesReceived}`
                  : `Telegram: disconnected${telegramStatus.lastError ? `\n${telegramStatus.lastError}` : ''}`
                }
              >
                ...
                <span className={`telegram-dot ${telegramStatus.connected ? 'green' : 'red'}`} />
              </span>
            )}
          </div>
        </div>
